...
function *primeGenerator(limit: number) {
  let iteration = 1;
  let prime = 2;
  let step = 0; 
  
  while((prime + step) < limit) {
     if(iteration == 1) { prime = 2; }
     else if(iteration == 2) { prime = 3; }
     else if(iteration == 3) {
       prime = 5;
       step = 2;
     } else {
     ...
     if((prime == 5) || (prime === 7) || (prime % 5 !== 0) && (prime % 7 !== 0)) { yield prime; }
     iteration += 1;
   }  
}
...
for(let n of test_prime) { console.log(n); }

let max_prime_factor = (value: number) => {
  let rest       = value;
  ...
  while(rest > breakpoint) {
    while(rest % prime !== 0) { prime = prime_gen.next().value; } 
